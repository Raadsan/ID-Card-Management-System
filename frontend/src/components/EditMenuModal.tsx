...
import { updateMenu, getMenuById } from "@/api/menuApi";
import { Loader2, Save, Trash2, Layout, Link as LinkIcon, Smile, ChevronDown } from "lucide-react";
...
const EditMenuModal: React.FC<EditMenuModalProps> = ({ isOpen, onClose, onSuccess, menuId }) => {
    ...
    const fetchMenuDetails = async () => {
        try {
            setFetching(true);
            const data = await getMenuById(String(menuId));
            setFormData({
                title: data.title || "",
                url: data.url || "",
                icon: data.icon || "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateMenu(String(menuId), {
                ...formData,
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Synchronize Navigation Node" maxWidth="max-w-2xl">
            {fetching ? (
