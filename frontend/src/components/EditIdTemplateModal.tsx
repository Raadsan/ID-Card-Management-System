...
const EditIdTemplateModal: React.FC<EditIdTemplateModalProps> = ({
    ...
    useEffect(() => {
        if (template) {
            ...
            if (template.layout) {
                // Determine if layout is string or object (prisma Json can be weird sometimes depending on client)
                // usually it is object.
                ...
                // Reset to defaults if no layout
                setPositions({
                    photo: { x: 100, y: 100, width: 150, height: 150 },
                    fullName: { x: 50, y: 300, fontSize: 24, color: "#000000" },
                    department: { x: 50, y: 340, fontSize: 18, color: "#666666" },
    ...
    const currentBg = showFront ? getPreviewBackground('front') : getPreviewBackground('back');
