...
import { Loader2, User, Mail, Phone, Lock, Camera, Shield, Users, ChevronDown, Save } from "lucide-react";
...
const EditUserModal: React.FC<EditUserModalProps> = ({
    ...
    const fetchInitialData = async () => {
        try {
            ...
            if (userData) {
                ...
                if (userData.photo) {
                    setPreviewUrl(`http://localhost:5000/uploads/${userData.photo}`);
                }
            }
        } catch (err) {
            console.error("Failed to fetch edit data:", err);
            setError("Connectivity error. Could not retrieve account details.");
        } finally {
            setFetching(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.fullName || !formData.email || !formData.roleId) {
            setError("Required: Full Name, Email, and Authority Rank.");
            return;
        }
        ...
        try {
            ...
            await updateUser(String(userId), submitData);
            onSuccess();
            onClose();
        } catch (err: any) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Modify System Account" maxWidth="max-w-2xl">
            <form onSubmit={handleSubmit} className="px-2 py-4">
                ...
                {fetching ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Syncing Profile...</span>
                    </div>
                ) : (
                    <div className="flex flex-col md:flex-row gap-12 items-start">
                        ...
                        <div className="flex flex-col items-center space-y-4 md:w-48 sticky top-0">
                            ...
                            <div className="text-center space-y-1">
                                <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em]">Update Avatar</p>
                                <p className="text-[9px] text-[#16BCF8] font-bold uppercase tracking-tight italic">Registry ID: #{userId}</p>
                            </div>
                        ...
                        <div className="flex-1 w-full space-y-5">
                            ...
                            {/* Password - Single Row (Optional during edit) */}
                            <div className="space-y-2">
                                <label className="text-[11px] font-black uppercase tracking-[0.05em] text-[#1B1555]/60 flex items-center gap-2">
                                    <Lock size={12} className="text-[#16BCF8]" /> New Password
                                </label>
                                <input
                                    type="password"
                                    name="password"
